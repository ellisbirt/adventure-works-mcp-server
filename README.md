...
`tools/list` advertises `get_customer_history`, `list_database_tables`, `read_database_table`, `get_product_profitability_summary`, `get_sales_trend_summary`, `get_customer_value_summary`, `get_product_mix_summary`, `get_geography_channel_summary`, `get_inventory_risk_summary`, and `get_demand_forecast_summary`. The seven sales-insight tools support optional filters (`top`, `startDate`, `endDate`, `productIds`, `productCategoryIds`) so business questions around margin, trend health, repeat customers, cross-sell opportunities, channel/geography variance, inventory risk, and near-term forecasting can be grounded without exposing row-level personal data. The catalog tool returns every user table with its useful columns. The read tool requires catalog-provided schema and table names and permits 1-100 rows. Credential and internal surrogate-key columns (e.g. password hashes, row GUIDs) are never returned; personal, contact, and location fields are returned but redacted and marked with a `[REDACTED]` value so their presence in the schema stays visible without leaking the underlying data.
